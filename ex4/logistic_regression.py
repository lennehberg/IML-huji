...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        """
        n_features = X.shape[1] if not self.include_intercept_ else X.shape[1] + 1
        if self.include_intercept_:
            X = np.hstack([np.ones((X.shape[0], 1)), X])

        # initialize base weights
        self.coefs_ = np.random.normal(loc=0.0,
                                       scale=1 / np.sqrt(n_features),
                                       size=n_features)

        # fit the gradient decent learner if not regularizing
        if self.penalty_ =="none":
            self.solver_.fit(f=LogisticModule(self.coefs_), X=X, y=y)
        else: # fit with regularization
            self.solver_.fit(f=RegularizedModule(fidelity_module=LogisticModule(self.coefs_),
                                                 regularization_module=L1(self.coefs_) if self.penalty_ == "l1"
                                                                                        else L2(self.coefs_),
                                                 lam=self.lam_,
                                                 weights=self.coefs_,
